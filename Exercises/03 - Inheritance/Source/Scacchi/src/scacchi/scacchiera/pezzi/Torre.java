	...
	public boolean mossaValida(Scacchiera scacchiera, Casella destinazione) {
		if (destinazione != null) {
			...
			if(deltaY==0){
				...
				for(int i=1;i<Math.abs(deltaX);i++){
					if(!scacchiera.getCasella(
						new Coordinata( this.getCasella().getCoordinata().getX() + i, this.getCasella().getCoordinata().getY() )).isEmpty()){
						return false;
					}
				}
			}
			if(deltaX==0){
				for(int i=1;i<Math.abs(deltaX);i++){
					if(!scacchiera.getCasella(
						new Coordinata(this.getCasella().getCoordinata().getX(), this.getCasella().getCoordinata().getY()+i) ).isEmpty()){
						return false;
					}
				}
			}
