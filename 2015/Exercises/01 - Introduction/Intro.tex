...
Questa lezione copre le  slides ``Classi come astrazioni" o in alternativa al capitolo  1- 2, 3, 4 e 5 del libro  Pellegrino Principe. “Java 8”.
...
Prima di discutere le variabili e i tipi di riferimento ricordiamo che la memoria in Java 
...
Le \emph{variabili}:
\begin{itemize}
\item consentono di accedere agli oggetti
\item sono allocate sullo stack a run-time quando si chiama il metodo su cui si sono dichiarate
\item sono deallocate quando il sottoprogramma ritorna al chiamante
\item gli \emph{oggetti} sono invece allocati sullo heap.
\end{itemize}
...
dell'applicazione. A discrezione della JVM viene eseguita l'operazione di ``\emph{garbage collection}'', che consiste nella rimozione degli oggetti non più utilizzabili dal programma per fare posto ad altri. Il componente che effettua tale operazione prende il nome di ``\emph{garbage collector}''.
...
Per dichiarare una variabile \`e sufficiente specificare il \emph{tipo} e un \emph{identificatore}, ovvero un nome simbolico utilizzato per riferirsi alla variabile\footnote{Java è case sensitive quindi caratteri in upper e lower case sono interpretati come caratteri differenti.}. \\
