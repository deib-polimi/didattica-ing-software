...
\textbf{regole base}, \textbf{regole avanzate} e \textbf{premade scenarios}.
In aggiunta vengono aggiunte le seguenti \textbf{regole addizionali}. 
...
implementate. La seguente \`e la spiegazione delle abbreviazioni nella
tabella.
...
persone.
Gli studenti di telecomunicazioni, possono fare i gruppi di un massimo
...
Le funzionalit\`a avanzate possono essere implementate dai tutti
...
\item nel caso in cui sia l'implementazione via socket che RMI sia richiesta all'avvio deve essere possibile adattare il metodo utilizzato rispetto al client
\end{itemize}
...
\item il cliente può riconnettersi e continuare il gioco se si sceglie di implementare il ``stateful client'' funzionalit\`a
\end{itemize}
